...
import (
	...
	"orders-tracker-cli/internal/pkg/mock_services"
	"os"
	"testing"
)
...
func TestCorreiosCLI_RetrieveOrder(t *testing.T) {
	...
	t.Run("Should print error message when don't find order", func(t *testing.T) {
		cli := CorreiosCLI{}
		cli.service = &mock_services.CorreiosServiceMock{}

		mock_services.CorreiosServiceMockFindOrderByNumber = func(orderNumber string) (*dto.CorreiosResponse, error) {
	...
	t.Run("Should print events description and date when there's an order", func(t *testing.T) {
		...
		mock_services.CorreiosServiceMockFindOrderByNumber = func(orderNumber string) (*dto.CorreiosResponse, error) {
			jsonFile, _ := os.Open("../mock_data/valid_code_with_one_event.json")
